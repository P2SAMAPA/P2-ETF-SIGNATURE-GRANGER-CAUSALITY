...
st.set_page_config(page_title="Signature Granger Causality", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🌀 Signature Granger Causality</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Path-dependent causality | Iterated integrals | Fully nonlinear Granger test</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Signature Granger")
...
st.sidebar.markdown(f"**Signature depth:** {config.SIG_DEPTH} | **Max lag:** {config.MAX_LAG}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'sig_granger_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Signature GC: {etf['sig_granger_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized GC"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized GC", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("🌀 Top ETFs by Signature Granger Causality (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Signature Granger causality** uses path signatures (iterated integrals).
        - Goes beyond linear Granger and transfer entropy.
        - Fully nonlinear, path‑dependent causality test.
        - Tests whether the signature of macro up to time t predicts ETF returns.
        - Higher score → macro strongly Granger‑causes ETF returns.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the signature GC scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Signature Granger Causality | Path-dependent nonlinear causality")
